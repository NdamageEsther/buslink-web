...
export default function Navbar() {
  ...
  // Prefix for authenticated users — keeps them inside their dashboard context
  function getDashboardPrefix() {
    if (isAdmin)    return '/admin';
    if (isAgency)   return '/agency';
    if (isDriver)   return '/driver';
    return '/passenger'; // PASSENGER or default
  }

  // Build nav links — authenticated users get dashboard-prefixed routes
  function buildNavLinks() {
    ...
    return [
      { label: 'Home',       path: prefix === '/admin' ? '/admin/dashboard' : prefix === '/agency' ? '/agency/dashboard' : prefix === '/driver' ? '/driver/dashboard' : '/home' },
      { label: 'Prices',     path: `${prefix}/prices` },
    ...
    <>
      <nav style={{
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '64px' }}>
          ...
          {/* Logo */}
          <Link
            to={isAuthenticated ? (isAdmin ? '/admin/dashboard' : isAgency ? '/agency/dashboard' : '/home') : '/'}
          ...
          <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }} className="bl-desktop">
            {navLinks.map(link => (
              <Link
                ...
                onMouseEnter={e => { if (!isActive(link.path)) { e.currentTarget.style.color = '#1d4ed8'; e.currentTarget.style.backgroundColor = '#f8fafc'; }}}
                onMouseLeave={e => { if (!isActive(link.path)) { e.currentTarget.style.color = '#475569'; e.currentTarget.style.backgroundColor = 'transparent'; }}}
              >
                {link.label}
              </Link>
            ))}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            {isAuthenticated ? (
              <>
                {/* User chip */}
                <div style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '5px 10px', backgroundColor: '#f1f5f9', borderRadius: '8px' }} className="bl-desktop">
                  <div style={{ width: '26px', height: '26px', backgroundColor: '#1d4ed8', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <span style={{ color: '#fff', fontSize: '11px', fontWeight: '700' }}>{user?.name?.charAt(0)?.toUpperCase() || 'U'}</span>
                  </div>
                  <div>
                    <div style={{ fontSize: '12px', fontWeight: '700', color: '#1e293b', whiteSpace: 'nowrap' }}>{user?.name}</div>
                    <div style={{ fontSize: '10px', color: '#64748b' }}>{user?.role?.replace('_', ' ')}</div>
                  </div>
                </div>
                <button
                  onClick={logout}
            ...
            {/* Hamburger */}
            <button onClick={() => setMobileOpen(o => !o)} style={{ padding: '8px', border: 'none', background: 'transparent', cursor: 'pointer' }} className="bl-mobile-only">
        ...
        {/* Mobile menu */}
        {mobileOpen && (
          <div style={{ borderTop: '1px solid #e2e8f0', padding: '12px 24px 16px', display: 'flex', flexDirection: 'column', gap: '2px', backgroundColor: '#fff' }}>
            {navLinks.map(link => (
              <Link key={link.path} to={link.path} style={{ padding: '11px 14px', borderRadius: '8px', fontWeight: isActive(link.path) ? '600' : '500', fontSize: '15px', textDecoration: 'none', color: isActive(link.path) ? '#1d4ed8' : '#475569', backgroundColor: isActive(link.path) ? '#eff6ff' : 'transparent' }}>
                {link.label}
              </Link>
            ))}
            <div style={{ borderTop: '1px solid #f1f5f9', marginTop: '8px', paddingTop: '12px' }}>
              {isAuthenticated
                ? <button onClick={logout} style={{ width: '100%', padding: '10px', backgroundColor: '#1d4ed8', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: '600', fontSize: '14px', cursor: 'pointer' }}>Logout</button>
                : <Link to="/login" style={{ display: 'block', padding: '10px', textAlign: 'center', border: '1.5px solid #1d4ed8', borderRadius: '8px', color: '#1d4ed8', fontWeight: '600', fontSize: '14px', textDecoration: 'none' }}>Login</Link>
              }
            </div>
          </div>
        )}
